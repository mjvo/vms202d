...
var RevealMenu = window.RevealMenu || (function(){
	...
	loadResource(options.path + 'menu.css', 'stylesheet', function() {
		if (loadIcons) {
			// changed to use Fontawesome CDN
			// loadResource(options.path + 'font-awesome/css/all.css', 'stylesheet', loadPlugin)
			loadResource('https://use.fontawesome.com/releases/v5.2.0/css/all.css', 'stylesheet', loadPlugin)
		} else {
			loadPlugin();
		}
	})
	...
	function loadPlugin() {
		...
		if (initialise) {
			...
			// modified var openButton = options.openButton;
			var openButton = options.open_button;
